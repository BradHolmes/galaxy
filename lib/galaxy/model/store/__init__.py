...
class ModelImportStore(metaclass=abc.ABCMeta):
    ...
    def _import_jobs(self, object_import_tracker, history):
        ...
        def _find_hda(input_key):
            ...
            if not hda:
                hda = self.sa_session.query(model.HistoryDatasetAssociation).get(input_key)
            return hda
        ...
        def _find_hdca(input_key):
            ...
            if not hdca:
                hdca = self.sa_session.query(model.HistoryDatasetCollectionAssociation).get(input_key)
            return hdca
        ...
        def _find_dce(input_key):
            ...
            if not dce:
                dce = self.sa_session.query(model.DatasetCollectionElement).get(input_key)
            return dce
